...
To recover :bdg-success:`Free Parameters` from our simulated data and to compare the recovered parameters to our true free parameters
...
.. dropdown:: The Logic of Recovering :bdg-success:`Free Parameters`
    ...
    Therefore, we would say that, if we know an individual subject's :bdg-danger:`Decisions`, we can calculate the :bdg-secondary:`Utility` that they would have experienced if that subject's true :bdg-success:`Free Parameters` were any of the possible coordinates in our parameter space ('observed :bdg-secondary:`Utility`'). 
    From there, we can compare the expected :bdg-secondary:`Utility` on each :bdg-primary:`Trial` for each of those :bdg-success:`Free Parameters` pairs to the observed utility on each :bdg-primary:`Trial` for each of those :bdg-success:`Free Parameters`. 
    Thus, recovering :bdg-success:`Free Parameters` is the process of drawing an inference - given that we know the outcome of the data generation process for each trial and the experimental parameters for each trial, we can infer about the unknowns - in this case the :bdg-success:`Free Parameters` which produced these :bdg-danger:`Decisions`. 
    
    We will infer that the :bdg-success:`Free Parameters` which generated the observed :bdg-danger:`Decisions` is the :bdg-success:`Free Parameter` pair where the difference between observed :bdg-secondary:`Utility` and expected :bdg-secondary:`Utility` are the smallest.
...
.. dropdown:: Objective Functions
    ...
    Here lower error means better model fit. 
...
.. dropdown:: About Optimizers

    Optimizers provide optimal solutions for certain functions. 
