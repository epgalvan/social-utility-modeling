...
.. dropdown::  Implication 1: We can ascertain how much Subjective Utility people derive from the decisions that they make.

    .. tab-set::
        .. tab-item:: Programming Logic

            :bdg-primary:`If` **Possible_Choices[which(U(Possible_Choices)) == max(U(Possible_Choices))]** :octicon:`arrow-right;1em` **Decision**

            :bdg-secondary:`Then` **U(Decision) = max(U(Possible_Choices))** 

            .. Note::
                ``U`` refers to Subjective Utility
