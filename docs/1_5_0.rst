    ...
    .. tab-set::
        ...
        .. tab-item:: R
            ...
            ::
                ...
                trialList = data.frame(Recipient = rep(c('Self', 'Other'), each = 100),
                                       MoneyA = 10,
        ...
        .. tab-item:: R
            ...
            ::
                ...
                for (i in 1:length(freeParameters[,1])){
                    ...
                    #Just Added
                    for (k in 1:length(trialList[,1])){
                        shocksThis = c(trialList$ShocksA[k], trialList$ShocksB[k])
                        shocksAlternative = c(trialList$ShocksB[k], trialList$ShocksA[k])
                        moneyThis = c(trialList$MoneyA[k], trialList$MoneyB[k])
                        moneyAlternative = c(trialList$MoneyB[k], trialList$MoneyA[k])
            ...
            ::
                ...
                for (i in 1:length(freeParameters[,1])){
                    ...
                    for (k in 1:length(trialList[,1])){
                        ...
                        Utility = vector('numeric', length(Choices))
                        for (n in 1:length(Choices)){
                            Utility[n] = utility(Payout = harm(shocksThis[n], shocksAlternative[n]),
                                                Harm = payout(moneyThis[n], moneyAlternative[n]),
                                                kappa = Kappa,
                                                lambda = Lambda)
                        }
                        correct_choice = which(Utility == max(Utility))
                        predictions[i,k] = Choices[correct_choice[sample(length(correct_choice), 1)]]
                    }
                }
